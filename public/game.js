...
socket.on("game_info", (data) => {
  ...
  if (data.gameSettings) {
    ...
    if (turnDurationValue)
      turnDurationValue.textContent = data.gameSettings.turnIntervalMs + "ms";
    if (turnDurationSlider)
      turnDurationSlider.value = data.gameSettings.turnIntervalMs;
  }
...
socket.on("game_update", (data) => {
  ...
  if (data.map_diff && data.map_diff.length > 0) {
    ...
    if (moveQueue.length > 0 && remainingMoves > 0) {
      ...
      const isAdjacent =
        Math.abs(queuedMove.fromTile - queuedMove.toTile) === 1 ||
...
socket.on("game_settings_updated", (settings) => {
  ...
  if (settings.turnIntervalMs !== undefined) {
    ...
    const turnDurationSlider = document.getElementById("turnDurationSlider");
    if (turnDurationValue)
      turnDurationValue.textContent = settings.turnIntervalMs + "ms";
...
function attemptMove(fromTile, toTile) {
  ...
  // Always queue the move, regardless of remainingMoves or queue state
...
function drawGame() {
  ...
  for (let i = 0; i < gameState.terrain.length; i++) {
    ...
    if (!isVisible) {
      ...
      const isEnemyGeneralTile =
        (gameState.generals && Object.values(gameState.generals).includes(i)) ||
        discoveredEnemyGenerals.has(i);

      if (isEnemyGeneralTile && terrain !== -2) {
